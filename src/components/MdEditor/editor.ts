...
import useOverlay from 'codemirror-ssr/addon/mode/overlay'
...
export async function handleImageUpload(
  { editor, appendBlock, codemirror }: AzimdEditorContext,
...
export function getBuiltinActions(
  ...
  console.log(plugins, uploadImages)
  // const items: AzimdAction[] = [
  //   {
  //     icon: icons.H({}),
  //     handler: {
  //       type: 'dropdown',
  //       actions: [1, 2, 3, 4, 5, 6].map((level) => ({
  //         title: locale[`h${level}` as keyof AzimdLocale],
  //         icon: [
  //           icons.H1({}),
  //           icons.H2({}),
  //           icons.H3({}),
