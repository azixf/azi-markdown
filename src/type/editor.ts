...
export interface AzimdViewerContext {
  /**
...
type Listener = (context: AzimdEditorContext) => void

type AzimdActionHandler =
  | {
      type: 'action'
      click: Listener
      /**
...
export interface AzimdAction {
   ...
   * Action handler
   */
  handler?: AzimdActionHandler
}

export interface AzimdPlugin {
  /**
   ...
   */
  editorEffect?(ctx: AzimdEditorContext): void | (() => void)
  /**
   * Side effect for the viewer, triggers when viewer props changes
   */
  viewerEffect?(ctx: AzimdViewerContext): void | (() => void)
}
