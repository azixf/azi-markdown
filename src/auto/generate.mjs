...
logger.info(
  '请输入要生成的页面或组件名称，如：page:user/component:user ，默认生成在pages或components目录下'
)
...
process.stdin.on('data', async (chunk) => {
  ...
  const name = inputs[1]
  const page_path =
    type === 'page' ? resolve('../pages', name) : resolve('../components', name)
  ...
  try {
    ...
    const export_name = `${name.slice(0, 1).toUpperCase()}${name.slice(1)}${
      type === 'page' ? 'Page' : 'Comp'
    }`
